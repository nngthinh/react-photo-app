...
const Home = () => {
  // States
  const isOnline = useSelector((state) => state.general.isOnline);
    ...
    const keepUserSession = async (isLoggedIn) => {
      if (isLoggedIn) {
        ...
        if (!getUserInfoResult.success) {
          ...
          if (
            String(getUserInfoResult.error.message).includes(
  ...
  return <HomeView isOnline={isOnline} />;
};

const HomeView = ({ isOnline }) => {
  return (
    <>
      <Navbar />
      {isOnline ? (
